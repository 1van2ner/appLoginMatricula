...
<div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h3 class="text-xl font-bold text-gray-900">Nueva Matrícula</h3>
            <p class="text-sm text-gray-500">Registra una nueva matrícula en el sistema.</p>
        </div>
    ...
    <form action="{{ route('enrollments.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="id_alumno" required class="mt-1 block w-full border rounded p-2">
                ...
                @foreach($students as $student)
                    <option value="{{ $student->id_alumno }}">{{ $student->nombre }} {{ $student->apellidos }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="id_teacher" class="mt-1 block w-full border rounded p-2">
                ...
                @foreach($teachers as $teacher)
                    <option value="{{ $teacher->id_profesor }}">{{ $teacher->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="id_schedule" class="mt-1 block w-full border rounded p-2">
                ...
                @foreach($schedules as $schedule)
                    <option value="{{ $schedule->id_schedule }}">Horario {{ $schedule->id_schedule }} - {{ $schedule->weekday }} {{ $schedule->start_time }} - {{ $schedule->end_time }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="semester" type="text" required class="mt-1 block w-full border rounded p-2" placeholder="2026-1" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Fecha de matrícula</label>
                <input name="enrollment_date" type="date" required class="mt-1 block w-full border rounded p-2" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="final_grade" type="number" step="0.01" min="0" max="20" class="mt-1 block w-full border rounded p-2" />
            </div>
            <div>
                ...
                <select name="status" required class="mt-1 block w-full border rounded p-2">
                    ...
                    <option value="ongoing" selected>Cursando</option>
                </select>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Guardar matrícula</button>
        </div>
    </form>
</div>
@endsection
